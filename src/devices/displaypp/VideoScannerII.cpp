...
void VideoScannerII::set_video_mode()
{
    ...
    if (supports_dblres())    // only on IIe/IIgs.
 ...
 * *** NEEDS UPDATE ***
 *
 *  Modifications since WmSimms first submitted this:
 *  - instead of calculating all this stuff every video_cycle, we pre-calculate a lookup table for each video mode.
 *  - a second lookup table is used to select the correct LUT based on the various video mode flags.
 *  - video_cycle then just references this LUT based on "current cycle count" inside a frame. (0-17029 NTSC, similar for PAL).
 *  - this works across II-II+-IIe-IIgs NTSC and PAL.
 *  - this is used to grab the correct byte from memory based on the virtual video scanner address;
 *  - that byte (and, for IIe, the aux byte) (and for IIgs, two successive SHR bytes or palette bytes or SCR byte) are queued linearly into
 *    a buffer which is then processed out by VideoScanGenerator into the bitstream representation.
 *  - Then the appropriate Monitor Renderer is called to render the bitstream into a frame.
 *
